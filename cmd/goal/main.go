...
var guideCommands = []guide.Command{
	...
	{
		Name:    "clean",
		Summary: "remove goal-generated .go files under a path (only files goal generated are deleted)",
		Usage:   "goal clean [-n] [path]",
		Flags:   []guide.Flag{{Name: "-n", Summary: "dry run: list the goal-generated files that would be removed, delete nothing"}},
	},
	{
		Name:    "ai",
...
func run(args []string, out, errOut io.Writer) error {
	...
	case "clean":
		path, dryRun, err := parseCleanFlags(rest)
		if err != nil {
			return usageErr(err)
		}
		return cmdClean(path, dryRun, out, errOut)
	case "run":
...
// parseCleanFlags pulls the -n (dry-run) flag and a single optional path
// (a directory to scan, default ".") out of args, mirroring parseFmtFlags. A
// trailing "/..." (or bare "...") is stripped, since the scan is already
// recursive.
func parseCleanFlags(args []string) (path string, dryRun bool, err error) {
	path = "."
	gotPath := false
	for _, a := range args {
		switch {
...
// cmdClean is the one-shot escape hatch: it walks the tree under path and
// removes every goal-generated .go file — a pure tree scan gated solely by the
// shared ownership matcher (IsGoalGenerated), so it needs no emit manifest and
// can never touch a hand-written .go or a file bearing a non-goal generator
// header (e.g. `// Code generated by protoc-gen-go`). With -n (dryRun) it lists
// what it would remove and deletes nothing. A per-file remove failure is
// reported to errOut but does not abort the walk or change the exit status; a
// missing path (walk root stat error) is returned so the command exits non-zero.
func cmdClean(path string, dryRun bool, out, errOut io.Writer) error {
	return filepath.WalkDir(path, func(p string, d fs.DirEntry, err error) error {
		...
		if err != nil {
			// A file we cannot read is one we cannot verify as goal-owned; skip it
			// rather than deleting blind or aborting the whole scan.
			fmt.Fprintln(errOut, "warning: could not read "+p+": "+err.Error())
			return nil
		}
		if !IsGoalGenerated(content) {
			return nil // hand-written, or a non-goal generator — never goal's to delete
		}
		if dryRun {
		...
		if err := os.Remove(p); err != nil {
			fmt.Fprintln(errOut, "warning: could not remove "+p+": "+err.Error())
			return nil
		}
