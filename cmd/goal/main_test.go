...
// TestEmitWritesGoalHeader proves every file goal build --emit writes carries the
// goal-owned generated-code header (US-001): a line-1 marker and a versioned
// line-2 generator line. The header is deterministic, so a re-emit is byte-identical.
func TestEmitWritesGoalHeader(t *testing.T) {
	dir := goalModule(t, map[string]string{"main.goal": mainGoal})
	...
	emitOnce := func() []byte {
		...
		if err != nil {
			t.Fatalf("reading emitted main.go: %v", err)
		}
		return b
	}
	...
	if len(lines) < 2 {
		t.Fatalf("emitted file has too few lines:\n%s", got)
	}
	if m := regexp.MustCompile(`^// Code generated by goal; DO NOT EDIT\.$`); !m.MatchString(lines[0]) {
		t.Errorf("line 1 = %q; want goal generated marker", lines[0])
	}
	if m := regexp.MustCompile(`^// generator: goal v.+$`); !m.MatchString(lines[1]) {
		t.Errorf("line 2 = %q; want versioned generator line", lines[1])
	}

	// A repeated emit of identical source is byte-identical (deterministic header).
	if again := emitOnce(); !bytes.Equal(got, again) {
		t.Errorf("re-emit not byte-identical:\nfirst:\n%s\nsecond:\n%s", got, again)
	}
}

// TestIsGoalGenerated proves the shared ownership predicate (US-002) matches
// goal-generated files by their line-1 marker, independent of the version on
// line 2, and rejects every non-goal or mispositioned marker — this is the
// single safety core reused by prune (US-005) and clean (US-006).
func TestIsGoalGenerated(t *testing.T) {
	const goalDev = "// Code generated by goal; DO NOT EDIT.\n" +
		"// generator: goal vdev\n\npackage main\n"
	const goalRelease = "// Code generated by goal; DO NOT EDIT.\n" +
		"// generator: goal v9.9.9\n\npackage main\n"
	const protoc = "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage pb\n"
	const stringer = "// Code generated by \"stringer -type=Pill\"; DO NOT EDIT.\n\npackage main\n"
	const handWritten = "package main\n\nfunc main() {}\n"
	// A goal-looking marker below the first line must NOT qualify.
	const markerBelow = "// some banner comment\n" +
		"// Code generated by goal; DO NOT EDIT.\n\npackage main\n"
	const crlf = "// Code generated by goal; DO NOT EDIT.\r\n" +
		"// generator: goal vdev\r\n\r\npackage main\r\n"
	...
	cases := []struct {
		...
		{"goal dev version", goalDev, true},
		{"goal release version", goalRelease, true},
		{"goal crlf line ending", crlf, true},
		{"protoc-gen-go header", protoc, false},
		{"stringer header", stringer, false},
		{"hand-written no header", handWritten, false},
		{"goal marker below line 1", markerBelow, false},
		{"empty content", "", false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := IsGoalGenerated([]byte(tc.content)); got != tc.want {
				t.Errorf("IsGoalGenerated(%q) = %v; want %v", tc.content, got, tc.want)
			}
		})
	}
}
...
// TestEmitPruneDeletesOrphans proves `goal build --emit --prune` (US-005) deletes
// the orphan set US-004 would only warn about, but scoped strictly to files goal
// itself generated: a goal-generated orphan is removed, while a co-located orphan
// bearing a non-goal (protoc) header and a hand-written .go with no header are
// left untouched even though a stale manifest names all three. After the run the
// manifest reflects only the current emit set, and the build still exits 0.
func TestEmitPruneDeletesOrphans(t *testing.T) {
	dir := goalModule(t, multiPkgFiles())
	...
	// Seed three prior-manifest entries the current source will NOT produce:
	//  - a goal-generated orphan (SHOULD be pruned),
	//  - a protoc-marked orphan (SHOULD survive — not goal-owned),
	//  - a hand-written file with no header (SHOULD survive).
	goalOrphan := filepath.Join(dir, "internal", "foo", "gone.go")
	if err := os.WriteFile(goalOrphan, []byte(goalGeneratedHeader("dev")+"package foo\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	protocOrphan := filepath.Join(dir, "internal", "foo", "thing.pb.go")
	if err := os.WriteFile(protocOrphan, []byte("// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage foo\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	handWritten := filepath.Join(dir, "internal", "foo", "plain.go")
	...
	augmented := append([]string{}, m.Files...)
	augmented = append(augmented, "internal/foo/gone.go", "internal/foo/thing.pb.go", "internal/foo/plain.go")
	if err := writeEmitManifest(dir, augmented); err != nil {
		t.Fatalf("rewriting manifest with orphans: %v", err)
	}

	// Second emit WITH --prune: exit 0, orphan deleted, non-goal files kept.
	out.Reset()
	errOut.Reset()
	if err := run([]string{"build", "--emit", "--prune", dir}, &out, &errOut); err != nil {
		t.Fatalf("prune emit failed (should exit 0): %v\nstderr: %s", err, errOut.String())
	}

	if _, err := os.Stat(goalOrphan); !os.IsNotExist(err) {
		t.Errorf("goal-generated orphan was not pruned (err=%v); it should be deleted", err)
	}
	if _, err := os.Stat(protocOrphan); err != nil {
		t.Errorf("protoc-marked orphan was deleted; only goal-generated files may be pruned: %v", err)
	}
	if _, err := os.Stat(handWritten); err != nil {
		t.Errorf("hand-written file was deleted; files without a goal header must never be pruned: %v", err)
	}

	// After --prune the manifest lists only the current emit set (no orphans).
	pruned, err := loadEmitManifest(filepath.Join(dir, ".goal-manifest.json"))
	if err != nil {
		t.Fatalf("loading post-prune manifest: %v", err)
	}
	for _, seeded := range []string{"internal/foo/gone.go", "internal/foo/thing.pb.go", "internal/foo/plain.go"} {
		if slices.Contains(pruned.Files, seeded) {
			t.Errorf("post-prune manifest still lists orphan %q; it should reflect only the current emit set", seeded)
		}
	}
	if !slices.Equal(pruned.Files, m.Files) {
		t.Errorf("post-prune manifest = %v; want the current emit set %v", pruned.Files, m.Files)
	}
}

// TestBuildPruneRequiresEmit proves `--prune` without `--emit` is a usage error
// (US-005): prune has nothing to act on without an emit, so it must not silently
// no-op.
func TestBuildPruneRequiresEmit(t *testing.T) {
	dir := goalModule(t, multiPkgFiles())

	var out, errOut bytes.Buffer
	err := run([]string{"build", "--prune", dir}, &out, &errOut)
	if err == nil {
		t.Fatal("`goal build --prune` without --emit should be a usage error, got nil")
	}
}

// TestParseCleanFlags proves the clean flag parser: -n sets the dry-run flag, a
// positional is the scan path (with a trailing "/..." stripped) defaulting to
// ".", and an unknown flag is rejected.
func TestParseCleanFlags(t *testing.T) {
	path, dryRun, err := parseCleanFlags([]string{"-n", "./pkg/..."})
	if err != nil {
		t.Fatal(err)
	}
	if !dryRun || path != "./pkg" {
		t.Errorf("parseCleanFlags = (dryRun=%v, path=%q), want (true, ./pkg)", dryRun, path)
	}
	path, dryRun, err = parseCleanFlags(nil)
	if err != nil {
		t.Fatal(err)
	}
	if dryRun || path != "." {
		t.Errorf("parseCleanFlags(nil) = (dryRun=%v, path=%q), want (false, .)", dryRun, path)
	}
	if _, _, err := parseCleanFlags([]string{"--nope"}); err == nil {
		t.Error("parseCleanFlags should reject an unknown flag")
	}
}

// TestCmdCleanRemovesOnlyGoalGenerated proves `goal clean` (US-006): in a tree
// with a goal-generated .go, a hand-written .go, and a protoc-marked .go, a real
// clean removes ONLY the goal-generated file (matcher-scoped) and exits 0, while
// `-n` (dry run) lists the goal file but deletes nothing.
func TestCmdCleanRemovesOnlyGoalGenerated(t *testing.T) {
	newTree := func(t *testing.T) (dir, goalGen, handWritten, protoc string) {
		t.Helper()
		dir = t.TempDir()
		...
		goalGen = filepath.Join(sub, "generated.go")
		if err := os.WriteFile(goalGen, []byte(goalGeneratedHeader("dev")+"package pkg\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		handWritten = filepath.Join(sub, "hand.go")
		if err := os.WriteFile(handWritten, []byte("package pkg\n\nfunc Hand() {}\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		protoc = filepath.Join(sub, "thing.pb.go")
		if err := os.WriteFile(protoc, []byte("// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage pkg\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		return dir, goalGen, handWritten, protoc
	}

	// Real clean: only the goal-generated file is removed; exit 0.
	dir, goalGen, handWritten, protoc := newTree(t)
	var out, errOut bytes.Buffer
	if err := run([]string{"clean", dir}, &out, &errOut); err != nil {
		t.Fatalf("goal clean should exit 0: %v\nstderr: %s", err, errOut.String())
	}
	if _, err := os.Stat(goalGen); !os.IsNotExist(err) {
		t.Errorf("goal-generated file was not removed (err=%v); clean must delete it", err)
	}
	if _, err := os.Stat(handWritten); err != nil {
		t.Errorf("hand-written file was removed; clean must never delete it: %v", err)
	}
	if _, err := os.Stat(protoc); err != nil {
		t.Errorf("protoc-marked file was removed; only goal-generated files may be cleaned: %v", err)
	}
	if !strings.Contains(out.String(), goalGen) {
		t.Errorf("clean output did not report the removed file %q:\n%s", goalGen, out.String())
	}

	// Dry run (-n): nothing is removed, the goal file is listed as would-remove.
	dir, goalGen, handWritten, protoc = newTree(t)
	out.Reset()
	errOut.Reset()
	if err := run([]string{"clean", "-n", dir}, &out, &errOut); err != nil {
		t.Fatalf("goal clean -n should exit 0: %v\nstderr: %s", err, errOut.String())
	}
	for _, p := range []string{goalGen, handWritten, protoc} {
		if _, err := os.Stat(p); err != nil {
			t.Errorf("dry run removed %q; -n must delete nothing: %v", p, err)
		}
	}
	if !strings.Contains(out.String(), "would remove") || !strings.Contains(out.String(), goalGen) {
		t.Errorf("dry run did not list the goal-generated file as would-remove:\n%s", out.String())
	}
}
...
func TestParseFlags(t *testing.T) {
	emit, jsonOut, prune, emitDir, root, err := parseFlags([]string{"--emit=out", "./pkg/..."})
	if err != nil {
		t.Fatal(err)
	}
	if !emit || jsonOut || prune || emitDir != "out" || root != "./pkg" {
		t.Errorf("parseFlags = (%v, %v, %v, %q, %q), want (true, false, false, out, ./pkg)", emit, jsonOut, prune, emitDir, root)
	}
	_, jsonOut, _, _, root, err = parseFlags([]string{"--json", "./pkg"})
	if err != nil {
		t.Fatal(err)
	}
	if !jsonOut || root != "./pkg" {
	...
	emit, _, prune, _, root, err = parseFlags([]string{"--emit", "--prune", "./pkg"})
	if err != nil {
		t.Fatal(err)
	}
	if !emit || !prune || root != "./pkg" {
		t.Errorf("parseFlags --emit --prune = (emit=%v, prune=%v, root=%q), want (true, true, ./pkg)", emit, prune, root)
	}
}

func TestParseFlagsUnknownFlag(t *testing.T) {
	// The legacy --engine flag was removed in US-043; it is now an unknown flag.
	if _, _, _, _, _, err := parseFlags([]string{"--engine=splice"}); err == nil {
