...
func TestGenerateCounts(t *testing.T) {
	...
	if transpile != 57 {
		t.Errorf("file-mode transpile pairs = %d, want 57", transpile)
	}
	...
	// qprop_assign_eq `?`-assignment case) also yields a doctest case.
	if doctest != 6 {
		t.Errorf("doctest cases = %d, want 6", doctest)
	}
	if other != 0 {
