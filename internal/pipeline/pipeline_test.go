...
// manifest through the default AST front-end (backend.Transpile) via the shared
// corpus runner, which gofmt-normalizes both sides before comparing. The cases
// — the testdata multi-feature programs and the per-feature reference examples
// the unified pipeline subsumes — live in the manifest, not in a directory list
// hardcoded here. It fails loudly if the manifest yields no transpile cases, so
// an empty or mis-generated manifest cannot masquerade as green.
func TestCorpusTranspile(t *testing.T) {
	...
	tp := corpus.TranspilerFunc(backend.Transpile)
	ran := 0
...
// through the default AST front-end (backend.Transpile) via the shared corpus
// doctest runner, which asserts the emitted `_test.go` sidecar (Output.Test)
// matches the golden after gofmt-normalizing both sides. It fails loudly if the
