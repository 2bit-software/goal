...
// appending to the implementor registry without duplicating an entry (so a union
// across a package's files stays a set).
...
func (info *Info) resolveTypeDecl(d *ast.GenDecl) {
	for _, s := range d.Specs {
		...
		case *ast.StructType:
			...
			// `type T struct implements I { … }` registers T as an implementor of I,
			// building the sealed-interface implementor registry (consulted for sealed
			// I; inert for ordinary I). The pointer form `*T` is the value a sealed
			// `match`/type-switch dispatches on, so store it as "*T".
			if t.Implements != nil {
				if iface := exprName(t.Implements.Type); iface != "" {
