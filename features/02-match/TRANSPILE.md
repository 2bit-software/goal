...
```go
switch __goal_v := <scrutinee>.(type) {
case <Enum>_<Variant>:
    <arm body, with bound fields rewritten to __goal_v.Field>
...
default:
...
- The type-switch guard variable is the hygienic `__goal_v` (§8 prefix). It is emitted **only when
...
- **Payload binding maps onto type-switch narrowing.** The bound name (`a` in `Status.Active(a)`)
  *is* the narrowed value, so it is rewritten to `__goal_v`, and field reads on it are exported:
  `a.since` → `__goal_v.Since` (the goal field name capitalized to the exported Go field from §8.1).
...
case Status_Cancelled:
	logEvent(__goal_v.Reason, __goal_v.At)
default:
...
4. **Binding + field rewrite.** Within an arm with binding `b` of variant `V`: every reference to
   `b` becomes `__goal_v`; a field read `b.f` where `f` is a declared field of `V` becomes
   `__goal_v.F` (exported). Non-field selectors on `b` keep their selector, only the base is
   renamed.
5. **Default.** If a `_` arm exists, its body is the `default`. Otherwise emit the defensive
   `panic("unreachable: non-exhaustive <Enum> (compiler invariant violated)")`.
6. **Guard variable.** Emit `__goal_v :=` only if step 4 produced at least one `__goal_v` reference;
...
The only synthesized temporary is the type-switch guard `__goal_v` (§8 prefix), emitted only when a
