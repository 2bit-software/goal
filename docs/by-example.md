...
**Nested `?`:** `?` also works *inside* an expression — a call argument, a binary
operand, or a return payload. Each nested `?` is **hoisted** onto its own statement
just before the enclosing one, bound to a fresh temp, and reuses the same
unwrap-or-early-return lowering. So `y := f(g()?)`:
...
**Evaluation order is preserved:** an operand evaluated before a nested `?` that has
a side effect (it calls a function) is bound first, so hoisting never reorders
effects. For `y := add(plain(), g()?)`, `plain()` binds before the `g()?` temp:
...
## Rejecting `?` on a short-circuit `&&` / `||`

A nested `?` is lifted (hoisted) to run *before* its enclosing statement, so it always
executes. On the conditionally-evaluated side of `&&` or `||` that would change behavior —
the operand should run only when the left side does not short-circuit. Rather than silently
hoisting it out of the guard, the transpiler rejects a `?` placed on the short-circuit side.

```goal name=qprop_shortcircuit.goal
package demo
...
func use(cond bool) Result[int, error] {
    if cond && (g()? > 0) {
        return Result.Ok(2)
    }
    return Result.Ok(0)
...
```error
backend: a `?` on the short-circuit side of && / || is not supported; bind it to a variable on its own line first
```

**Why:** `?` unconditionally early-returns a failure, but the right side of `&&` (and `||`)
runs only when the left side does not short-circuit. Hoisting the `?` out of the guard would
evaluate `g()` even when `cond` is false, changing behavior. Bind it first — `x := g()?` on
its own line, then test `cond && x > 0` — so the propagation is explicit and unconditional.
